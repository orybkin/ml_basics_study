...
wandb.login()
if args.dset_type == 1: experiment = 'q3b_shapes_'
else: experiment = 'q3b_mnist_'
...
tokenizer = ColorQuantizeTokenizer(n_colors, image_shape)
...
samples = model.sample(bos_onehot, length=image_length, deterministic=False)
samples = tokenizer.detokenize(samples)
...
image_grid = torchvision.utils.make_grid(torch.Tensor(image / channel * 255).permute(0,3,1,2)).permute(1,2,0).cpu().numpy().astype(np.uint8)
